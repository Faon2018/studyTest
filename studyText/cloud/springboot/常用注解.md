...
bean的名称和方法名称相同，你也可以使用name属性来指定
...
public class AppConfig {
    ...
    @Description("Provides a basic example of a bean")//bean的描述
...
//当未使用@Component注解时，需要在启动类中加上@EnableConfigurationProperties注解
...
@RestController
@RequestMapping("/api")
//由于验证的是Person对象，该对象的属性验证已设置，故不需要添加@Validated注解
...
@RestController
@RequestMapping("/api")
@Validated//由于参数上的验证规则是临时设置（在方法的参数后直接设置，而不是应用已经设置验证规则的对象），所以需要添加@Validated注解
public class PersonController {

    @GetMapping("/person/{id}")//参数注解Valid
